...
public class Prj3Part2_WithOut_MRV {
	private int iteration = 0;
	public static void main(String[] args) {
		// TODO Auto-generated method stub
		Prj3Part2_WithOut_MRV test = new Prj3Part2_WithOut_MRV();
	...
	public void playGame() {
		...
		/*for(HashSet<String> row:domains)
			System.out.println(row);*/
		
		if (backtracking2(0,table, domains, 0)){
	...
	private boolean backtracking2(int position, String[][] table, HashSet<String>[] domainsTable, int layer) {
		...
		//domainsTable[row] = nextDomains;
		//System.out.println(nextDomains);
		...
		for(String val:nextDomains){
			...
			/*if(position == 21 || position==24){
				...
				if(position == 21){
					helper(position,table);
					System.out.println("is: "+consistency_check_job(position, table));
				}
			...
			if(consistency_check_job(position, table)){
				/*if(table[2][0]=="Water")
					System.out.println("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&");*/
				if(position==24||backtracking2(position+1, table, domainsTable, layer+1)){
		...
		//System.out.println("Fail to select value for postion: "+row+" "+col);
	...
	public boolean consistency_check_job(int position, String[][] table) {
		...
		/*if(table[2][1]=="Tea"){
			...
			System.out.println(position/5 +" "+position%5);
		}*/
		int row = position/5;
		int col = position%5; 
			...
			case 4://pet
				switch(table[row][col]){
					...
					case "Dog":
						...
						break;
					case "Horse":
						//System.out.println("compare hourse");
						if(col==0){
							...
							if((table[3][col-1]!="" && table[3][col-1]!="Kits Kats") && (table[3][col+1]!="" && table[3][col+1]!="Kits Kats")){
								/*System.out.println((table[3][col-1]!="" && table[3][col-1]!="Kits Kats"));
								System.out.println((table[3][col+1]!="" && table[3][col+1]!="Kits Kats"));
								System.out.println((table[3][col-1] +" "+table[3][col-1]));
								System.out.println((table[3][col+1] +" "+table[3][col+1]));
